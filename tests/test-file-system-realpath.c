...
static mmux_asciizcp_t	ptn_asciiz_original      = "./test-file-system-realpath.d/../test-file-system-realpath.ext";
static mmux_asciizcp_t	ptn_asciiz_directory     = "./test-file-system-realpath.d";
static mmux_asciizcp_t	ptn_asciiz_canonicalised = "./test-file-system-realpath.ext";
...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-file-system-realpath";
  ...
  /* Do it. */
  {
    ...
    /* Retrieve the canonicalised pathname. */
    {
      ...
      printf_message("realpath-ing");
      if (mmux_libc_realpath(fs_ptn_canonicalised, fs_ptn_factory, fs_ptn_original)) {
	printf_error("realpath-ing");
	handle_error();
      }
    }
	...
	printf_message("realpath pathname exists as a directory entry");
      } else {
	printf_error("realpath pathname does NOT exist");
	...
	printf_message("realpath pathname is a regular file");
      } else {
	printf_error("realpath pathname is NOT a regular file");
	mmux_libc_exit_failure();
      }
    }
    ...
    /* Final cleanup. */
    {
      ...
      mmux_libc_file_system_pathname_final(fs_ptn_canonicalised);
    }
  }
