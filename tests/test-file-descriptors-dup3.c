...
static mmux_asciizcp_t	ptn_asciiz = "./test-file-descriptors-dup3.ext";
...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-file-descriptors-dup3";
...

  {
	...
	if (mmux_libc_open(fd1, fs_ptn, flags, mode)) {
      ...
      {
	auto	flags = mmux_libc_open_flags(0);

	mmux_libc_make_fd(fd2, 11);

	printf_message("dup3-ing");
	if (mmux_libc_dup3(fd2, fd1, flags)) {
	  printf_error("dup3-ing");
	  handle_error();
	}
      }
