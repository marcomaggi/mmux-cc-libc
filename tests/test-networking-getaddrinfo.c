...

static void
test_getaddrinfo_with_localhost_hints (void)
{
  printf_message("testing: %s", __func__);
  {
    bool			there_is_one_more;
...

static void
initialise_hints_addrinfo_with_poste_it (mmux_libc_addrinfo_t addrinfo)
{
  /* Initialise the field ai_family. */
  {
    if (mmux_libc_addrinfo_family_set(addrinfo, MMUX_LIBC_AF_UNSPEC)) {
  ...
  /* Initialise the field ai_canonname. */
  {
    if (mmux_libc_addrinfo_canonname_set(addrinfo, "poste.it")) {
      handle_error();
    }
  }
...

static void
test_getaddrinfo_with_poste_it_hints (void)
{
  printf_message("testing: %s", __func__);
  {
    bool			there_is_one_more;
    ...
    mmux_libc_addrinfo_t	ai_next;
    mmux_libc_gai_errno_t	gai_errno;
    mmux_asciizcp_t		node = "poste.it", service = "http";
    mmux_libc_addrinfo_t	hints_addrinfo;

    initialise_hints_addrinfo_with_poste_it(hints_addrinfo);
