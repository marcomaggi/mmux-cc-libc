...
static mmux_asciizcp_t	ptn_asciiz = "./test-file-system-faccessat2.ext";
...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-file-system-faccessat2";
...

  {
    ...
    if (true) {
      ...
      printf_message("faccessat2-ing first time");
      if (mmux_libc_faccessat2(&access_is_permitted, dirfd, fs_ptn, how, flags)) {
	printf_error("faccessat2-ing first time");
	handle_error();
    ...
    if (true) {
      ...
      printf_message("faccessat2-ing second time");
      if (mmux_libc_faccessat2(&access_is_permitted, dirfd, fs_ptn, how, flags)) {
	printf_error("faccessat2-ing second time");
	handle_error();
      } else if (! access_is_permitted) {
