...
static mmux_asciizcp_t		src_pathname_asciiz = "./test-file-system-faccessat2.src.ext";
...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  if (1) {
    ...
    printf_message("faccessat2ing first time");
  ...
  if (1) {
    ...
    printf_message("faccessat2ing second time");
    if (mmux_libc_faccessat2(&access_is_permitted, dirfd, ptn, how, flags)) {
      handle_error();
    } else if (! access_is_permitted) {
