...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-wait-any-process";
  }
  ...
  if (mmux_libc_fork(&this_is_the_parent_process, &child_pid)) {
    ...
    /* Wait for the child process. */
    {
      ...
      if (mmux_libc_wait_any_process(&process_completion_status_available,
	...
	if (process_completion_status_available) {
	  MMUX_LIBC_IGNORE_RETVAL(mmux_libc_dprintfer("%s: parent process: child process completion status: %d\n",
	  ...
	  MMUX_LIBC_IGNORE_RETVAL(mmux_libc_dprintfer("%s: parent process: no complete child process status\n", PROGNAME));
	  mmux_libc_exit_failure();
	}
      }
    }
  } else {
    MMUX_LIBC_IGNORE_RETVAL(mmux_libc_dprintfer("%s: child process: exiting\n", PROGNAME));
    mmux_libc_exit_success();
  }
