...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-file-descriptors-readv-writev";
  }

  {
	...
	printf_message("writev-ing");
	if (mmux_libc_writev(&nbytes_done, mfd, &iova)) {
	  printf_error("writev-ing");
	  handle_error();
	}
	...
	printf_message("readv-ing");
	if (mmux_libc_readv(&nbytes_done, mfd, &iova)) {
	  printf_error("readv-ing");
	  handle_error();
	}
	...
	for (mmux_standard_uint_t i=0; i<bufnum.value; ++i) {
	  for (mmux_standard_uint_t j=0; j<buflen.value; ++j) {
	    auto	expected_octet = mmux_octet(10 * i + j);
	    if (false) {
