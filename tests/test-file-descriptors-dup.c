...
static mmux_asciizcp_t	ptn_asciiz = "./test-file-descriptors-dup.ext";
...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-file-descriptors-dup";
...

  {
	...
	if (mmux_libc_open(fd1, fs_ptn, flags, mode)) {
      ...
      {
	printf_message("dup-ing");
	if (mmux_libc_dup(fd2, fd1)) {
	  printf_error("dup-ing");
	  handle_error();
	}
      }
