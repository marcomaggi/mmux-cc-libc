...
/* Build the IPv6 sockaddr to be used by both the client and the server. */
{
  ...
  mmux_libc_sockaddr_ipsix_port_set(sockaddr, mmux_libc_network_port_number_from_host_byteorder_literal(8080));
...
server_doit (mmux_libc_pid_t child_pid,
	     mmux_libc_sockaddr_ipsix_t server_sockaddr, mmux_libc_socklen_t server_sockaddr_length)
{
  ...
  /* Build the server socket. */
  {
    ...
    if (mmux_libc_socket(server_sockfd, MMUX_LIBC_PF_INET6, MMUX_LIBC_SOCK_STREAM, MMUX_LIBC_IPPROTO_TCP)) {
...

static void
client_doit (mmux_libc_sockaddr_ipsix_t server_sockaddr, mmux_libc_socklen_t server_sockaddr_length)
{
  ...
  /* Build the client socket. */
  {
    ...
    if (mmux_libc_socket(client_sockfd, MMUX_LIBC_PF_INET6, MMUX_LIBC_SOCK_STREAM, MMUX_LIBC_IPPROTO_TCP)) {
...
main (int argc MMUX_CC_LIBC_UNUSED, char const *const argv[] MMUX_CC_LIBC_UNUSED)
{
  ...
  {
    mmux_cc_libc_init();
    PROGNAME = "test-networking-sockets-stream-ipsix-accept";
  }
  ...
  /* Do it. */
  {
    ...
    printf_error("wrong return to main function");
    mmux_libc_exit_failure();
  }
}
